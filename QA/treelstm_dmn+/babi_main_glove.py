...
class InputModule(nn.Module):
    ...
    def forward(self, contexts, word_embedding, tree_dict):
        ...
        # print("contexts_list_len",len(contexts_list_hidden))
        def pad_zoro_tensor(contexts_list_hidden):
            lengths = []
            ...
            for i in range(len(contexts_list_hidden)):
                contexts_list_hidden[i] = F.pad(contexts_list_hidden[i],
                                                (0, 0, 0, max_size - contexts_list_hidden[i].size(0)), 'constant',
                                                0)
            return contexts_list_hidden

        contexts_list = pad_zoro_tensor(contexts_list_hidden)
            ...
            def load_glove(glove_path, vocab_size, word_dict):
                ...
                for word in word_dict.keys():
                    if glove_vocab.getIndex(word):
                        emb[word_dict[word]] = glove_emb[glove_vocab.getIndex(word)]
                return emb

            emb = load_glove(glove_path, vocab_size, dset.tree_dict)
            model.word_embedding.weight.requires_grad_(True)

            # torch.save(emb, emb_file)
            # plug these into embedding matrix inside model

            model.word_embedding.weight.data.copy_(emb)
