...
def get_unindexed_qa(raw_babi):
    ...
    for i, line in enumerate(babi):
        ...
        if line.find('?') == -1:
            ...
            task["Q"] = ' '.join(jieba.cut(translate.translate(line[:idx], dest='zh-CN').text))
            # task["Q"] = line[:idx]
            task["A"] = translate.translate(tmp[1].strip(), dest='zh-CN').text
            # task["A"] = tmp[1].strip()
            task["S"] = [] # Supporting facts
            for num in tmp[2].split():
                task["S"].append(id_map[int(num.strip())])
            tc = task.copy()
            # tc['C'] = translate.translate(tc['C'].split('<line>')[:-1], dest='zh-CN').text
